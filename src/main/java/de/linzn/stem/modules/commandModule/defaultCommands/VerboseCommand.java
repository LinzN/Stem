...
public class VerboseCommand implements ICommand {
    ...
    public boolean executeTerminal(String[] args) {
        boolean value = STEMApp.logSystem.getLogLevel() == Level.ALL;
        value = !value;
        STEMApp.LOGGER.LIVE("Set verbose to " + value);
        STEMApp.logSystem.setLogLevel(value ? Level.ALL : STEMApp.getInstance().getConfiguration().logLevel);
        return true;
    }
